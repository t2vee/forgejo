...
test('Test External Release Attachments', async ({browser, isMobile}, workerInfo) => {
  test.skip(isMobile);
  ...
  await expect(page.locator('.download[open] li:nth-child(3)')).toContainText('Test');
  await expect(page.locator('.download[open] li:nth-child(3) a')).toHaveAttribute('href', 'https://forgejo.org/');
  save_visual(page);
  ...
  await expect(page.locator('.download[open] li:nth-child(3)')).toContainText('Test2');
  await expect(page.locator('.download[open] li:nth-child(3) a')).toHaveAttribute('href', 'https://gitea.io/');
  await expect(page.locator('.download[open] li:nth-child(4)')).toContainText('Test3');
  await expect(page.locator('.download[open] li:nth-child(4) a')).toHaveAttribute('href', 'https://gitea.com/');
  save_visual(page);
  await page.locator('.octicon-pencil').first().click();
